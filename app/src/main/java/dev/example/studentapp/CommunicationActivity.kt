...
package dev.example.studentapp
...
import dev.example.studentapp.chatlist.ChatListAdapter
import dev.example.studentapp.models.ContentModel
import dev.example.studentapp.network.Client
import dev.example.studentapp.network.NetworkMessageInterface
import dev.example.studentapp.network.Server
import dev.example.studentapp.peerlist.PeerListAdapter
import dev.example.studentapp.peerlist.PeerListAdapterInterface
import dev.example.studentapp.wifidirect.WifiDirectInterface
import dev.example.studentapp.wifidirect.WifiDirectManager
...
class CommunicationActivity : AppCompatActivity(), WifiDirectInterface, PeerListAdapterInterface,
    ...
    private var wfdDevice: WifiP2pDevice? = null
    ...
    private var plainTextId: String? = " "
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val connectedMessage: TextView = findViewById(R.id.tvConnected)
        val attendance = wfdDevice?.deviceName
        connectedMessage.text = "Currently Attending: $attendance"
    }
    ...
    fun discoverNearbyPeers(view: View) {
        val studentId: EditText = findViewById(R.id.etStudentId)

        val studentID = studentId.text.toString()
        val studentIdArr: Array<String> = studentID.toCharArray().map { it.toString() }.toTypedArray()
        if (studentIdArr[0] == "8" && studentIdArr[1] == "1" && studentIdArr[2] == "6" && studentId!=null) {

            wfdManager?.discoverPeers(studentId)
            plainTextId = studentId.text.toString()
        } else {
            val toast = Toast.makeText(this, "Id is invalid", Toast.LENGTH_SHORT)
            toast.show()
        }
    }
    ...
    override fun onGroupStatusChanged(groupInfo: WifiP2pGroup?) {
        ...
        if (groupInfo == null){
            ...
            client = Client(this, plainTextId.toString())
            deviceIp = client!!.ip
        }
    }
